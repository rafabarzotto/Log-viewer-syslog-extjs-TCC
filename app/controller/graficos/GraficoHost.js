Ext.define('Log.controller.graficos.GraficoHost', {
    ...
    init: function(application) {
        this.control({
            "graficos tabgraficos graficohost tabpanel": { // Alias GRID!
                render: this.onWindowRender
            },
            "graficos tabgraficos graficohost tabpanel menu#changeType menuitem": {
                click: this.onChangeChart
            },
            "graficos tabgraficos graficohost tabpanel menu#download menuitem": {
    ...
    onWindowRender: function(panel, eOpts) {
        var panel = Ext.ComponentQuery.query('graficos tabgraficos graficohost tabpanel > hostspie')[0];
        panel.getStore().load();
    },

    onChangeChart: function(item, e, options) {
        //var panel = item.up('graficos');
        var panel = Ext.ComponentQuery.query('graficos tabgraficos graficohost tabpanel')[0];
    ...
    onChartDownload: function(item, e, options) {
        ...
        var chart = Ext.ComponentQuery.query('graficos tabgraficos graficohost tabpanel')[0].getLayout().getActiveItem();
